...
def get_link_ratio(x):
    ...
    for key in comment_dict.keys():
        output_dict[key] = link_dict[key]/comment_dict[key]
    input_file.close()
    output_file.close
    return output_dict 
...
def make_video_data_files(x):
    ...
    for k,v in x.items():
        ...
        for link in v:
            try: 
                video = pafy.new(link)
                output_file.write("{0},{1}\n".format(video.title.replace(",",""), video.viewcount))
            except:
                continue
        output_file.close()
...
def get_avg_views(x):
    ...
    for k in x.keys():
        ...
        while True:
            ...
            count += int(line.split(',')[comma_count])
            index += 1
        if(index < 30):
            count = 0
...
print({k: v for k, v in sorted(y.items(), key=lambda item: item[1])})
'''
...
'''
# #Test ESTP's categories distribution
# y = get_link_categories(youtube_links)
# print({k: v for k, v in sorted(y.items(), key=lambda item: item[1])})
